...
export default function OptionScreen({
  ...
  const resultsPerSearchConfigs = {
    ...
    setOptionValue: (value: number | string) => {
      setOptions({ resultsPerSearch: Number(value) });
    },
  };
  ...
  const startingNumberConfigs = {
    ...
    setOptionValue: (value: string | number) => {
      setOptions({ startingNumber: value as 'selected' | '0' | '1' });
    },
  };
  ...
  function parseNOfSearchValue(value: string): number {
    ...
    if (Number.isNaN(tempValue)) parsedValue = 24;
      ...
      <div className="option-container">
        <InputOption
          ...
          setOptionValue={(value: string) => {
            setOptions({ numberOfSearches: parseNOfSearchValue(value) });
          }}
        />
        <SelectOption
...
function SelectOption({
  ...
  return (
    <div className="option">
      <h3 className="option-name">{optionName}</h3>
      <div onClick={optionBoxOnClick} className="option-box">
        {currentValue}
        {optionTransition(
          (style, show) =>
            show && (
