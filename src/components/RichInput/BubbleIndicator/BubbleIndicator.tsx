...
export default function BubbleIndicator({
  ...
  const svgNewWidth =
    SVG_DEFAULT_WIDTH + bubbleState.length + EXTRA_HEIGHT * newAspectRatio;
  ...
  function onMouseUp() {
    ...
    dispatchBubbleState({
      ...
      payload: hoveringSpan.current ?? null,
    });
  ...
  function onMouseMove(e: MouseEvent) {
    draggingHandler(e);
    if (bubbleIndicatorRef.current)
      bubbleIndicatorRef.current.style.animation = 'none';
  }
  ...
  useEffect(() => {
    setSelectedSpan(
      bubbleState.spanToAttach ?? numberInputSpans.at(-1) ?? null
    );
  ...
  return (
    <animated.svg
      ...
      viewBox={bubbleAnimation.length.to(
        (value) =>
          `${0} 0 ${
            SVG_DEFAULT_WIDTH + (value <= -17 ? -17 : value)
          } ${SVG_DEFAULT_HEIGHT}`
      )}
    >
      <animated.path
