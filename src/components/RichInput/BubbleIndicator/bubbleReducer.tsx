...
type BubbleReducerAction =
  ...
  | { type: 'setBubbleVisibility' | 'setIsDragging'; payload: boolean }
...
function getBubbleStateOnSpan(
  ...
  if (numberInputSpans.length === 0 || !bubble?.parentElement) return {};

  const currentSpan =
    spanToAttach || numberInputSpans[numberInputSpans.length - 1];
  ...
  const top =
    ((spanRect.y - parentRect.y + spanRect.height / 2) / parentRect.height) *
    100;
  const left =
    ((spanRect.x - parentRect.x + spanWidth / 2 - bubbleWidth / 2) /
      parentRect.width) *
    100;
